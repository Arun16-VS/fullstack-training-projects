...
@Component({
      ...
      <div class="container nav-inner">
        <a routerLink="/questions" class="brand">
          <span class="brand-icon" aria-hidden="true">
            ...
            <span class="brand-node right"></span>
              ...
              <div class="avatar">
                <img
                  *ngIf="currentUser?.profilePicture"
                  ...
                  [alt]="(currentUser?.username || 'User') + ' profile photo'"
                  class="avatar-image">
                <span *ngIf="!currentUser?.profilePicture">{{ currentUser?.username?.charAt(0)?.toUpperCase() }}</span>
              </div>
  ...
  styles: [`
    ...
    .brand-icon {
      position: relative; width: 24px; height: 20px; display: inline-flex;
    ...
    .brand-node.top { top: 0; left: 9px; }
    .brand-node.left { bottom: 1px; left: 1px; }
    .brand-node.right { bottom: 1px; right: 1px; }
    .brand-link {
    ...
    .notif-btn.has-notif { border-color: var(--accent); }
